...
// Commitments are the member's own work, which is what `commitments.own.manage`
// says: OWN for Owner, Admin and Member, and absent for Support. Gated on the
// capability so the matrix is the thing enforced rather than a role list that
// happens to agree with it today.
actionRouter.use(authenticate, tenantContext, requireCapability("commitments.own.manage"), idempotency);
