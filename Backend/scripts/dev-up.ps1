...
function Test-ApiScoped {
  try {
    ...
    # A seeded account may not exist in every database; that is not a scope
    # problem, so say so rather than reporting a false failure.
    return @{ Scoped = $null; Detail = $_.Exception.Message }
  }
}
...
Write-Step '2/3  api (from source)'
